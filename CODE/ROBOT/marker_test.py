...
last_marker_drop_time = 0
MARKER_DROP_COOLDOWN = 5.0
...
i2c_marker = busio.I2C(board.SCL, board.SDA)

pca_marker = PCA9685(
    i2c_marker,
    address=MARKER_PCA_ADDR
)

pca_marker.frequency = 50
# audio shared
mic_person = 0
mic_dog = 0
mic_noise = 0
...
def set_marker_servo(ch, angle):
    ...
    pca_marker.channels[ch].duty_cycle = pulse << 4

# =====================================================

def marker_open_door():

    print("[MARKER] OPEN DOOR")
...
def fusion_loop():
    ...
    while running:
        ...
        # =================================
        # DROP MARKER ONLY WHEN CAMERA SEES PERSON
        # =================================
        ...
        if (
            ...
            not marker_busy
            and
            now - last_marker_drop_time >= MARKER_DROP_COOLDOWN
        ):

            print("[MARKER] PERSON DETECTED")

            last_marker_drop_time = now

            Thread(
                target=drop_marker_once,
                daemon=True
            ).start()
